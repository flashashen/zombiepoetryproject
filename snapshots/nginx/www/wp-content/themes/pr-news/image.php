					...
					<div class="panel-body">
						<?php if (has_post_thumbnail()): ?>
							<div class="post-thumbnail img-thumbnail">
								<?php the_post_thumbnail('post-image-' . prpin_get_imagespan(), array('class' => 'img-responsive')); ?>
							</div>
						<?php endif; ?>
						<div class="page-header">
							...
							<?php echo prpin_get_post_meta(true, true, true, false, false) ?><?php
							$metadata = wp_get_attachment_metadata();
							printf(__('<div class="post-meta">Full size: <a href="%1$s" title="Link to full-size image">%2$s &times; %3$s</a> in <a href="%4$s" title="Return to %5$s" rel="gallery">%6$s</a></div>', 'prpin'), esc_url(wp_get_attachment_url()), $metadata['width'], $metadata['height'], esc_url(get_permalink($post->post_parent)), esc_attr(strip_tags(get_the_title($post->post_parent))), get_the_title($post->post_parent)
							);
							?>
							<?php edit_post_link(__('Edit', 'prpin'), '<span class="edit-link">', '</span>'); ?>
						</div>
						...
						<ul class="pager">
							<li class="previous">
								<?php previous_image_link(false, '<i class="fa fa-chevron-left"></i> ' . __('Previous', 'prpin') . '</span>'); ?>
							</li>
							<li class="next">
								<?php next_image_link(false, ' ' . __('Next', 'prpin') . ' <i class="fa fa-chevron-right"></i></span>'); ?>
							</li>
						</ul>
